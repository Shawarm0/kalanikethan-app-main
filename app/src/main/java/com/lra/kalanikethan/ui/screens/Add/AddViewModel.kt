...
class AddViewModel(
    ...
    fun createStudent() {
        _students.update { students -> newStudent(students) }
    }

    fun deleteStudent(student: Student) {
        _students.update { students -> removeStudentFromList(students, student) }
    }

    fun updateStudent(index: Int, newStudent: Student) {
        _students.update { students -> updateStudentInList(students, index, newStudent) }
    }

    fun createParent() {
        _parents.update { parents -> newParent(parents) }
    }

    fun deleteParent(parent: Parent) {
        _parents.update { parents -> removeParentFromList(parents, parent) }
    }

    fun updateParent(index: Int, newParent: Parent) {
        _parents.update { parents -> updateParentInList(parents, index = index, newParent) }
    }
    ...
    fun createFamily(paymentData: PaymentData, parents: List<Parent>, students: List<Student>) {
        ...
        viewModelScope.launch {
            try {
                id = familyRepository.addFamily(family)
                ...
                paymentRepository.addPaymentData(plan)
                for (parent in parents) {
                    familyRepository.addParent(parent.copy(familyId = id as String))
                }
                for (student in students) {
                    studentRepository.addStudent(student.copy(familyId = id as String))
                }
                paymentRepository.addFirstFamilyPayment(paymentData.paymentId, date, paymentData.amount.toFloat())
            } catch (e: Exception) {
